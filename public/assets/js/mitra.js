$(document).ready(function(){
    ...
    $('.btn-edit-mitra').click(function(){
        const modal = new bootstrap.Modal(document.getElementById('editMitraModal'));
        $('#id').val($(this).data('id'));
        ...
        $('#hargaMitraEdit').val('Rp '+ Number($(this).data('harga')).toLocaleString('id-ID'));
        $('#editMitraModal select[name="lokasiMitra"]').val($(this).data('lokasi'));
        $('#editMitraModal select[name="keahlianMitra"]').val($(this).data('keahlian'));
        $('#editMitraModal textarea[name="deskripsiMitra"]').val($(this).data('deskripsi'));
    ...
    $('#editMitraForm').submit(function(e){
        ...
        formData.set('hargaMitra', formData.get('hargaMitra').replace(/[^0-9]/g,''))

        axios.post(`/admin/edit-mitra/${id}?_method=PUT`, formData)
            .then(res=>{
    ...
    $('#addMitraForm').submit(function(e){
        ...
        axios.post('/admin/tambah-mitra', formData)
            .then(res=>{
                Swal.fire({icon:'success', title:'Berhasil', text:res.data.message, timer:1500, showConfirmButton:false})
                    .then(()=>location.reload());
            })
    ...
    $('.form-delete-mitra').submit(function(e){
        ...
        Swal.fire({
            ...
            if(result.isConfirmed){
                axios.delete(`/admin/hapus-mitra/${id}`)
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil',
