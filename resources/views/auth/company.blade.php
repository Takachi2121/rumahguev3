<form class="company-regis-form form-hidden" data-form="company">

    <input type="text" class="form-control input-login" placeholder="Masukkan Nama Perusahaan...">
    <input type="text" class="form-control input-login" placeholder="Masukkan Alamat Perusahaan...">
    <input type="email" class="form-control input-login" placeholder="Masukkan Email Perusahaan...">

    <div class="password-input">
        <input type="password" id="password" class="form-control input-login" placeholder="Masukkan Password...">
        <i class="fa-regular fa-eye-slash togglePasswordRegis"></i>
    </div>

    <div class="password-input">
        <input type="password" id="passwordRepeat" class="form-control input-login" placeholder="Masukkan Ulang Password...">
        <i class="fa-regular fa-eye-slash togglePasswordRepeat"></i>
    </div>

    <div class="form-extra">
        <a href="#" class="text-decoration-none text-muted small">Lupa Password?</a>
    </div>

    <button type="submit" class="btn btn-login w-100">Daftar</button>

    <div class="divider">
        <hr>
            <span>Atau Masuk Menggunakan</span>
        <hr>
    </div>
    <div class="d-flex justify-content-center">
        <a href="" class="text-decoration-none text-muted"><i class="fa-brands fa-google fa-2x"></i></a>
    </div>
    <p class="fw-light mt-3 text-center">Sudah Punya Akun? <a href="" class="text-decoration-none text-danger fw-semibold ">Login</a></p>
</form>
